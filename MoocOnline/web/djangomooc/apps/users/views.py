...
class RegisterView(View):
    def get(self, request):
        ...
        all_courses = Course.objects.all().order_by("-students")[:3]
...
class LoginView(View):
    def get(self, request):
        all_courses = Course.objects.all().order_by('-click_nums')[:3]
        return render(request, "login.html", {
            "all_courses":all_courses,
        })
...
class IndexView(View):
    ...
    def get(self, request):
        ...
        # 取出首页中部左上方课程轮播图
        banner_courses = Course.objects.filter(is_banner=False)[7:10]
...
class ForgetPwdView(View):
    def get(self, request):
        ...
        all_courses = Course.objects.all().order_by('-fav_nums')[:3]
...
class UserInfoView(LoginRequireMixin, View):
    ...
    def post(self, request):
        # 修改信息须添加 instance 参数，否则 save 时会新增一条数据
...
class UploadImageView(LoginRequireMixin, View):
    ...
    def post(self, request):
        ...
        if image_form.is_valid():
            # 图片保存在内存中，验证通过后在 image_form 下的 cleaned_data 中
            # 使用 modelform 直接保存
            image_form.save()
            return HttpResponse('{"status":"success"}', content_type='application/json')
        else:
